...
class Agent_AGRP(object):
    ...
    def __init__(self, NAME):
        ...
        self.SOLV_POINT_ALL0 = {};self.SOLV_POINT_ALL1 = {};
        # --- Models ---
        self.SERCHING_STEP = 5;
        self.ITEMS_MQ_PART = None;
        self.MODEL_AGRP_HIDDEN_STATE_SP = None;
        self.MODEL_ML_BAYESIAN_FITTER = None;
        self.POLICY_WEIGHT = np.random.rand(8);

    def Q_function(self,SOLUTION_PART,SOLUTION_PART_VEC):
        SP01_POINTS = compute_special_points(EQUATIONS_MQ,SOLUTION_PART,self.SOLV_POINT_ALL0,self.SOLV_POINT_ALL1);
        HIDDEN_STATE = self.MODEL_AGRP_HIDDEN_STATE_SP.predict([ np.array([self.ACTION_VEC],dtype=float),np.array([SP01_POINTS[0]],dtype=float),np.array([SP01_POINTS[1]],dtype=float) ])[0];
        Q_VALUE = self.MODEL_ML_BAYESIAN_FITTER.predict([HIDDEN_STATE]);
        Q_VALUE = output_processing_ml_bayesian(Q_VALUE);
        return Q_VALUE;

    def init_solution(self):
        self.ACTION,self.ACTION_VEC = generating_random_solv(self.ITEMS_VARS_SUBS);
        for VARIABLE in self.ITEMS_VARS_USED:self.SOLV_POINT_ALL0[VARIABLE] = 0;
        for VARIABLE in self.ITEMS_VARS_USED:self.SOLV_POINT_ALL1[VARIABLE] = 1;

    def policy_function(self):
        return int(np.exp(np.dot(np.array(self.ACTION_VEC),self.POLICY_WEIGHT)))%len(self.ALL_SOLUTIONS);

    def policy_function_update(self):
        DELTA = 1.0 - self.Q_VALUE_NOW;
        for INDEX_i in range(self.POLICY_WEIGHT.shape[0]):
            for INDEX_j in range(len(ITEMS_BOOLEAN)*NUM_RAND_SEED):GAMMA = np.random.rand(1)*0.001;
            self.POLICY_WEIGHT[INDEX_i] += GAMMA*DELTA*self.SERCHING_STEP*self.ACTION_VEC[INDEX_i];

    def Pi_function(self,EQUATIONS_MQ):
        if len(self.VISITED_POINTS) == 0:
            self.VISITED_POINTS.append(self.ACTION);
            return self.ACTION;
        self.SERCHING_STEP = self.policy_function();
...
AGENT.MODEL_ML_BAYESIAN_FITTER = MODEL_ML_BAYESIAN_FITTER;AGENT.MODEL_AGRP_HIDDEN_STATE_SP = MODEL_AGRP_HIDDEN_STATE_SP;
