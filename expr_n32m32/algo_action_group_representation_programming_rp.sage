...
class Agent_AGRP(object):
    ...
    def __init__(self, NAME):
        ...
        self.EQUATIONS_MQ_PART = None;
        # --- Models ---
        self.SERCHING_STEP = 5;
        self.ITEMS_MQ_PART = None;
        self.MODEL_AGRP_HIDDEN_STATE_RP = None;
        self.MODEL_AGRP_STATE_TO_VALUE_RP = None;
        self.MODEL_AGRP_GR_MAT_RP = None;
    ...
    def Q_function(self,EQUATIONS_MQ,SOLUTION_PART_VEC):
        ...
        self.SOLV_POINT_ALLY = [POLYNOMIAL.subs(SOLUTION_PART_RAND) for POLYNOMIAL in self.EQUATIONS_MQ_PART];
        HIDDEN_STATE = self.MODEL_AGRP_HIDDEN_STATE_RP.predict([ np.array([self.ACTION_VEC],dtype=float),np.array([ self.SOLV_POINT_ALLX ],dtype=float),np.array([ self.SOLV_POINT_ALLY ],dtype=float) ])[0];
        MOVE_VEC = (np.array(self.ACTION_VEC,dtype=float) + np.array(SOLUTION_PART_VEC,dtype=float))%2;
        MOVE_MAT = self.MODEL_AGRP_GR_MAT_RP.predict(np.array([MOVE_VEC]))[0];
        NEXT_STATE = np.dot(MOVE_MAT,HIDDEN_STATE);
        Q_VALUE = output_processing_cnn(self.MODEL_AGRP_STATE_TO_VALUE_RP.predict(np.array([NEXT_STATE]))[0]);
        return Q_VALUE;
...
from model_agrp_gr_rp import *;
MODEL_AGRP_GR_MAT_RP.load_weights('MODEL_AGRP_RHO_RP_weights.h5');
...
AGENT.MODEL_AGRP_HIDDEN_STATE_RP = MODEL_AGRP_HIDDEN_STATE_RP;AGENT.MODEL_AGRP_STATE_TO_VALUE_RP = MODEL_AGRP_STATE_TO_VALUE_RP;AGENT.MODEL_AGRP_GR_MAT_RP = MODEL_AGRP_GR_MAT_RP;
...
while ENV_STATE.IS_SAT==False:
    ...
    AGENT.EQUATIONS_MQ_PART = EQUATIONS_MQ_PART;
