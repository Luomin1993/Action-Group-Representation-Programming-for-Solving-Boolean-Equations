...
#include <map>
#include <ctime>
// g++ app_particles_swarm_opt.cpp -o app -lginac -lcln
...
float value_function(int const& SAT_NUM_NOW)
{
    return sin(SAT_NUM_NOW/float(MAX_EQ_NUM));
}
...
int main(int argc, char const *argv[])
{
    ...
    GiNaC::exmap SOLUTION_NEXT;float FEEDBACK_IN_HISTORY = 0.4;float FEEDBACK_NOW = 0.4;
    ...
    while(true)
    {
        ...
        IS_UPDATED = false;FEEDBACK_NOW = value_function(SAT_NUM_NOW);
        if (FEEDBACK_NOW + FEEDBACK_IN_HISTORY>1.0)
        {
            cout<<"--- Dense Feedback Position ---"<<endl;
            FEEDBACK_IN_HISTORY = FEEDBACK_NOW;
