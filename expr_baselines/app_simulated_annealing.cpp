...
#include <map>
#include <ctime>
// g++ app_simulated_annealing.cpp -o app -lginac -lcln
...
bool do_jumping(int const& delta_m)
{
    //cout<<"delta_m = "<<delta_m<<endl;
    //cout<<"rand()%10/10.0 = "<<rand()%10/10.0<<endl;
    //cout<<"1/exp(5/delta_m) = "<<1/exp(5/delta_m)<<endl;
    float prob_jumping = 1/exp(5/delta_m);
    if (prob_jumping > rand()%10/10.0){return true;}
    return false;
}
...
int main(int argc, char const *argv[])
{
    ...
    while(true)
    {
        ...
        for (int INDEX_i = 0; INDEX_i < MAX_USED_VARS_NUM; ++INDEX_i)
        {
            ...
            {
                SOLUTION_NOW = SOLUTION_NEXT;
                SAT_NUM_NOW = SAT_NUM_NEXT;
                IS_UPDATED = true;break;
            }
        ...
        if(SAT_NUM_NOW == MAX_EQ_NUM){break;}
        if(do_jumping(MAX_EQ_NUM - SAT_NUM_NOW))
        {
            cout<<" --- do jumping --- "<<endl;
            IS_VISITED = true;
            while(IS_VISITED)
            {
                SOLUTION_NOW = generating_rand_solv(ITEMS_VARS_USED);
