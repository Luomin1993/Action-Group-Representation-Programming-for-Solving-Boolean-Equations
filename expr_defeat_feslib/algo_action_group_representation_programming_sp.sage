...
class Agent_AGRP(object):
    ...
    def __init__(self, NAME):
        ...
        self.MODEL_AGRP_STATE_TO_VALUE_SP = None;
        self.MODEL_AGRP_GR_MAT_SP = None;

    def Q_function(self,EQUATIONS_MQ,SOLUTION_PART_VEC):
        SP01_POINTS = compute_special_points(EQUATIONS_MQ,self.ACTION,self.SOLV_POINT_ALL0,self.SOLV_POINT_ALL1);
        ...
        MOVE_MAT = self.MODEL_AGRP_GR_MAT_SP.predict(np.array([MOVE_VEC]))[0];
        NEXT_STATE = np.dot(MOVE_MAT,HIDDEN_STATE);
        Q_VALUE = output_processing_cnn(self.MODEL_AGRP_STATE_TO_VALUE_SP.predict(np.array([NEXT_STATE]))[0]);
        return Q_VALUE;
    ...
    def Pi_function_rand(self,EQUATIONS_MQ):
        ...
        Q_VALUE = self.Q_function(EQUATIONS_MQ,SOLUTION_PART_VEC);
...
from model_agrp_gr_sp import *;
MODEL_AGRP_GR_MAT_SP.load_weights('MODEL_AGRP_RHO_SP_weights.h5');

# ============================== Agent Solving Boolean Equations Interactively ===================================
NUM_VARS_USED = 40;
NUM_EQUATIONS = 40;
NUM_VARS_SUBS =  8;
EQUATIONS_MQ = get_equations_from_fes_format_in('dataset/EQUATIONS_n40_m40_F0.in');
...
AGENT.MODEL_AGRP_HIDDEN_STATE_SP = MODEL_AGRP_HIDDEN_STATE_SP;AGENT.MODEL_AGRP_STATE_TO_VALUE_SP = MODEL_AGRP_STATE_TO_VALUE_SP;AGENT.MODEL_AGRP_GR_MAT_SP = MODEL_AGRP_GR_MAT_SP;
