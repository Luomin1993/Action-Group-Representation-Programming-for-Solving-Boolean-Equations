...
# ========= load dataset ================
DAT_STATE_ACTIONS   = np.load('DAT_SP_POINTS_VALUE_FIELD/DAT_STATE_ACTIONS.npy',allow_pickle=True);
DAT_STATE_CODE_NOW  = np.load('DAT_SP_POINTS_VALUE_FIELD/DAT_STATE_CODE_NOW.npy',allow_pickle=True);
DAT_STATE_CODE_NEXT = np.load('DAT_SP_POINTS_VALUE_FIELD/DAT_STATE_CODE_NEXT.npy',allow_pickle=True);
...
# ========= define MODEL_AGRP_RHO_SP ============
MODEL_AGRP_RHO_SP = Model(inputs=[f_input_mat,f_input_state], outputs=f_state_out);
MODEL_AGRP_GR_MAT_SP = Model(inputs=f_input_mat, outputs=f_mat_out);
sgd           = optimizers.SGD(learning_rate=0.001, decay=0.0, momentum=0.4, nesterov=True);
MODEL_AGRP_RHO_SP.compile(optimizer=sgd, loss=losses.mean_squared_error, metrics=['mae']);

# ========= Training MODEL_AGRP_RHO_SP ================
# MODEL_AGRP_RHO_SP.fit([DAT_STATE_ACTIONS,DAT_STATE_CODE_NOW],DAT_STATE_CODE_NEXT,epochs=15,batch_size=30);
# MODEL_AGRP_RHO_SP.save_weights('MODEL_AGRP_RHO_SP_weights.h5');

# ========= Running MODEL_AGRP_RHO_SP ================
# MODEL_AGRP_RHO_SP.load_weights('MODEL_AGRP_RHO_SP_weights.h5');
# DAT_STATE_CODE_NEXT_ = MODEL_AGRP_RHO_SP.predict([DAT_STATE_ACTIONS[0:10],DAT_STATE_CODE_NOW[0:10]]);
