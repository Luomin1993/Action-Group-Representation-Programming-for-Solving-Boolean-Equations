...
def output_processing_cnn(DAT_VALUE_PREDICTED):
    ...
    if len(DAT_VALUE_PREDICTED)==1: return (DAT_VALUE_PREDICTED[0]-0.05)*5.3;
    ...
    for INDEX_i in range(len(DAT_VALUE_PREDICTED)):
        DAT_VALUE_PREDICTED[INDEX_i] = (DAT_VALUE_PREDICTED[INDEX_i] - MIN_OUT)*5.3;
...
# plt.plot(SOLV_POINTS, DAT_VALUE_PREDICTED, marker='o', mec='r', mfc='w',label=u'Solutions Value Field');
# plt.plot(SOLV_POINTS, DAT_VALUE_TRUE, marker='x', mec='b', mfc='w',label=u'Solutions Value Field');
# plt.ylim(-0.2, 1.1);
...
# NUM_UPPER_BOUND = 18000;DAT_STATE_ACTIONS = [];DAT_STATE_CODE_NOW = [];DAT_STATE_CODE_NEXT = [];
